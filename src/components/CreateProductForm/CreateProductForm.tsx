...
const CreateProductForm: FC = () => {
    ...
    const addProductColor = (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        if (hexValue && colorName) {
            setProductColorObj((prevState) => [...prevState, {hexValue, colorName}]);
            hex.value = '';
            nameHex.value = '';
        }
    }
    ...
    return (
        <form onSubmit={handleSubmit(submitForm)}>
            ...
            <div className={'container'}>
                ...
                <button onClick={addProductColor} disabled={true}>Add Color</button>
            </div>
            ...
            <div>
                ...
                <div>{formErrors.price && <div>Error age: {formErrors.price[0]}</div>}</div>
                <div>{formErrors.priceSign && <div>Error email: {formErrors.priceSign[0]}</div>}</div>
                <div>{formErrors.imageLink && <div>Error phone: {formErrors.imageLink[0]}</div>}</div>
                <div>{formErrors.description && <div>Error password: {formErrors.description[0]}</div>}</div>
                <div>{formErrors.rating && <div>Error password: {formErrors.rating[0]}</div>}</div>
            </div>
        </form>
    );
};
